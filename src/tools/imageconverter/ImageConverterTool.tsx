...
export default function ImageConverterTool() {
    ...
    (file: File, fmt: OutputFormat, q: number): Promise<Blob> => {
      return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
          ...
          canvas.toBlob(
            (blob) => { if (!blob) { reject(new Error('Conversion failed')); return; } resolve(blob); },
  ...
  const handleConvert = useCallback(async () => {
    ...
    for (const img of pending) {
      setImages((prev) => prev.map((i) => (i.id === img.id ? { ...i, status: 'converting' as const } : i)));
      try {
        const blob = await convertImage(img.originalFile, outputFormat, quality);
        setImages((prev) => prev.map((i) => i.id === img.id ? { ...i, convertedBlob: blob, convertedSize: blob.size, status: 'done' as const } : i));
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : t('imageConverter.conversionError');
        setImages((prev) => prev.map((i) => i.id === img.id ? { ...i, status: 'error' as const, error: message } : i));
      }
      completed++;
      setProgress(Math.round((completed / total) * 100));
    }
    setStatus('done');
  }, [images, quality, outputFormat, convertImage, t]);
  ...
  if (status === 'done' && doneImages.length > 0) {
    const outputFiles = doneImages
      ...
      .map((i) => {
        ...
        return new File([i.convertedBlob], `${baseName}${ext}`, { type: getMimeType(outputFormat) });
      });
      ...
      <div className={styles.container}>
        <div className={styles.resultSection}>
          <div className={styles.resultSummary}>
            ...
            <h3>
              {doneImages.length === 1
                ? t('imageConverter.successSingle', { format: getFormatLabel(outputFormat) })
                : t('imageConverter.success', { count: String(doneImages.length), format: getFormatLabel(outputFormat) })}
            </h3>
          </div>
          <ToolSuccess
            ...
            crossLinks={[
              { icon: '🗜️', label: t('imageConverter.compressLink'), href: localizedHref('/image/compress-image') },
              { icon: '✨', label: t('imageConverter.svgLink'), href: localizedHref('/image/png-to-svg') },
              { icon: '🪄', label: t('imageConverter.bgLink'), href: localizedHref('/image/remove-background') },
            ]}
          />
          <button className={styles.resetButton} onClick={handleReset}>{t('imageConverter.convertMore')}</button>
        </div>
        ...
        <div className={styles.loadingSection}>
          <div className={styles.progressWrapper}>
            ...
            <p className={styles.statusText}>
              {t('imageConverter.converting', { name: images.find((i) => i.status === 'converting')?.originalFile.name ?? '...' })}
            </p>
  ...
  return (
    <div className={styles.container}>
      <div className={styles.privacyBadge}>
        <ToolIcon name="shield" size={14} /> {t('imageConverter.privacyBadge')}
      </div>
      ...
      >
        <div className={styles.dropzoneContent}>
          ...
          <p className={styles.dropzoneTitle}>
            {images.length === 0 ? t('imageConverter.dropTitle') : t('imageConverter.dropTitleMore')}
          </p>
          <p className={styles.dropzoneSubtitle}>{t('imageConverter.dropSubtitle')}</p>
          <button className={styles.uploadButton}>{t('imageConverter.chooseFiles')}</button>
        </div>
        <input
          ref={fileInputRef} type="file" accept={ACCEPT_STRING} multiple
        ...
        <>
          <div className={styles.controlsPanel}>
            <div className={styles.controlGroup}>
              <div className={styles.controlRow}>
                <span className={styles.controlLabel}>{t('imageConverter.convertTo')}</span>
                <div className={styles.formatRow}>
                  {(['jpeg', 'png', 'webp'] as OutputFormat[]).map((fmt) => (
                    ...
                    >{getFormatLabel(fmt)}</button>
                  ))}
                </div>
              </div>
              {isLossy && (
                <div className={styles.controlRow}>
                  <span className={styles.controlLabel}>{t('imageConverter.quality')}</span>
            ...
            {images.map((img) => (
              <div key={img.id} className={styles.fileItem}>
                ...
                <div className={styles.fileInfo}>
                  ...
                  <span className={styles.fileMeta}>{formatSize(img.originalSize)} → {getFormatLabel(outputFormat)}</span>
                </div>
          ...
          <div className={styles.actionBar}>
            <span className={styles.actionSummary}>
              {images.length === 1 ? t('imageConverter.image') : t('imageConverter.images', { count: String(images.length) })} ·{' '}
              {formatSize(images.reduce((s, i) => s + i.originalSize, 0))}
            </span>
            <button className="btn btn-primary btn-lg" onClick={handleConvert} disabled={images.length === 0}>
              {t('imageConverter.convertButton', { format: getFormatLabel(outputFormat) })}
            </button>
          </div>
        </>
      )}
