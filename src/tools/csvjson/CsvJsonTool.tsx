    ...
    <div className={styles.container}>
      <div className={styles.modeToggle}>
        <button className={`${styles.modeBtn} ${mode === 'csv2json' ? styles.modeActive : ''}`}
          onClick={() => { setMode('csv2json'); setOutput(''); setError(''); }}>CSV → JSON</button>
        <button className={`${styles.modeBtn} ${mode === 'json2csv' ? styles.modeActive : ''}`}
          onClick={() => { setMode('json2csv'); setOutput(''); setError(''); }}>JSON → CSV</button>
      </div>
      <div className={styles.layout}>
        <div className={styles.col}>
          <span className={styles.colTitle}>{mode === 'csv2json' ? 'CSV Input' : 'JSON Input'}</span>
        ...
        <div className={styles.col}>
          <div className={styles.colHeader}>
            <span className={styles.colTitle}>{mode === 'csv2json' ? 'JSON Output' : 'CSV Output'}</span>
